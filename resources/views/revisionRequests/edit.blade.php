...
@section('content')
    ...
    <div class="container mx-auto px-3">
        ...
        <!-- Content -->
        <div>
            <form action="{{ route('revisionRequests.update', [$file, $revisionRequest]) }}" method="POST" id="updateForm" enctype="multipart/form-data">
                @csrf
                @method("PUT")
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-3 items-start">
                    <div class="bg-white rounded-xl shadow-md p-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="technicianFirstName" id="technicianFirstName" class="block rounded-md border-0 bg-gray-100 focus:ring-2 w-full"
                                   placeholder="{{ __('First name') }}" value="{{ old('technicianFirstName', $revisionRequest->technicianFirstName) }}">
                        ...
                        <div>
                            ...
                            <input type="text" name="technicianLastName" id="technicianLastName" class="block rounded-md border-0 bg-gray-100 focus:ring-2 w-full"
                                   placeholder="{{ __('Last name') }}" value="{{ old('technicianLastName', $revisionRequest->technicianLastName) }}">
                        ...
                        <div>
                            ...
                            <input type="email" name="technicianEmail" id="technicianEmail" class="block rounded-md border-0 bg-gray-100 focus:ring-2 w-full"
                                   placeholder="{{ __('E-mail') }}" value="{{ old('technicianEmail', $revisionRequest->technicianEmail) }}">
                    ...
                    <div>
                        <div class="bg-white rounded-xl shadow-md p-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="name" id="name" class="block rounded-md border-0 bg-gray-100 focus:ring-2 w-full"
                                       placeholder="{{ __('Name') }}" value="{{ old('name', $revisionRequest->name) }}">
                                ...
                                <select name="categoryId" id="categoryId" class="block rounded-md border-0 bg-gray-100 focus:ring-2 w-full">
                                    @foreach ($categories as $categoryEntry)
                                        <option
                                            ...
                                            @if(old('categoryId', $revisionRequest->categoryId) == $categoryEntry->id) selected @endif>
                                            {{ $categoryEntry->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <textarea name="description" id="description" rows="6" placeholder="{{ __('Description of the change') }}" class="block rounded-md border-0 bg-gray-100 focus:ring-2 w-full">{{ old('description', $revisionRequest->description) }}</textarea>
                        ...
                        <div class="flex justify-end flex-grow gap-2 w-full md:w-auto mt-3">
                            <a href="{{ route('revisionRequests.show', ['file' => $file, 'revisionRequest' => $revisionRequest]) }}"
                            ...
                            <a href="javascript:$('#updateForm').submit();"
